...
export type WorkflowRole = (typeof WorkflowRole)[keyof typeof WorkflowRole];
...
const LEGACY_STEP_MAP: Record<string, WorkflowRole> = {
  HOD: WorkflowRole.HOD,
  FINANCE: WorkflowRole.FINANCE_DIRECTOR,
  GM: WorkflowRole.GENERAL_MANAGER,
  SUPERVISOR: WorkflowRole.SUPERVISOR,
  EMPLOYEE: WorkflowRole.EMPLOYEE,
};

const VALID_ROLES = new Set<string>(Object.values(WorkflowRole));
const VALID_STEP_TYPES = new Set<string>(WORKFLOW_STEP_TYPES.map((t) => t.type));

export function normalizeLegacyRole(value: string): WorkflowRole | null {
  if (VALID_ROLES.has(value)) return value as WorkflowRole;
...
export function createApprovalStep(role: WorkflowRole = WorkflowRole.HOD, label?: string): WorkflowApprovalStep {
  return {
    id: createStepId(),
    type: 'APPROVAL',
    role,
  ...
  switch (type) {
    case 'PARALLEL':
      return {
        ...
        steps: [createApprovalStep(WorkflowRole.HOD), createApprovalStep(WorkflowRole.FINANCE_DIRECTOR)],
      };
    case 'READ':
      return { id: createStepId(), type: 'READ', role: WorkflowRole.SUPERVISOR, action: 'READ', label: 'Must read' };
    case 'EDIT':
      return { id: createStepId(), type: 'EDIT', role: WorkflowRole.EMPLOYEE, action: 'EDIT', label: 'Must edit' };
    case 'NOTIFY':
      return { id: createStepId(), type: 'NOTIFY', role: WorkflowRole.SUPERVISOR, label: 'Notification' };
    case 'CONDITION':
      return { id: createStepId(), type: 'CONDITION', label: 'Condition', expression: '' };
    case 'SIGN':
    case 'APPROVAL':
    default:
      return createApprovalStep(WorkflowRole.HOD);
  }
}
...
export function flattenApprovalRoles(steps: WorkflowStep[]): WorkflowRole[] {
